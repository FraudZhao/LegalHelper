...
#3.4复选框
rootdir=os.getcwd()
lawsfolderdir=rootdir+'/'+'法律'
folderlist=os.listdir(lawsfolderdir)#法律种类文件夹列表
...
def classification(lawlistRV,typeoflaw): #分类函数,lawlistRV为该函数的局部变量，切勿与复选框的lawlist混淆
    ...
    for law in lawlistRV:
        if '.DS_Store' in law or 'DS' in law or law.startswith('.'): #苹果会对访达打开过的文件创建这些隐藏文件，需要略过
            continue
...
for folder in folderlist: #不同类型法律放在不同文件夹中
    singlelawfolderdir=lawsfolderdir+'/'+folder #单个法律文件夹地址列表
    if '.DS_Store' in folder: #苹果会在访达打开过的文件夹创建这个文件，需要略过
        continue
    if os.path.isdir(lawsfolderdir+'/'+folder)==False: #法律文件夹下不能直接放文件
        text1.insert(0.0,"请注意，“法律”文件夹下不能直接放docx文档。\n请将docx文档放入相应的法律类别文件夹中。") 
        break
    singletypelawslist=os.listdir(singlelawfolderdir)
    singletypelawslist.sort()#先初排一遍，这样解释1、解释2、解释3就不会乱
    lawlist.append(classification(singletypelawslist,folder))
else:
    del singletypelawslist
 
if system=='Darwin':#创建listbox，mac和windows的宽度不一样。Darwin即macos
    lb1=Listbox(root,bg='white',bd=0,selectmode='multiple',width='16')
if system=='Windows':
    lb1=Listbox(root,bg='white',bd=0,selectmode='multiple',width='20')
...
for singletypelawslist in lawlist:
    ...
    for law in singletypelawslist:
        lb1.insert('end',law[1+law.index('/'):law.index('.')])
    ...
    if location in contentdict: #该法典之前检索过
        ...
        for paragraph in file.paragraphs: #生成以条文为元素的列表content
            ...
            if len(paragraph.text)==0:
                continue
            if '、' in paragraph.text[:4]: #一些司法解释会以一、二、分节
                continue
...
def but1():
    ...
    selectedindextuple=lb1.curselection()#组建选中项目的列表selectedlist
    ...
    for singletypelawslist in lawlist:
        for law in singletypelawslist:
            if law[1+law.index('/'):law.index('.')] in selectedlist: #选中了该法律
                artlist=articlelist(lawsfolderdir+'/'+law) #对选中的法律进行法典转化
                result.write('\n'+law+'\n')
