...
for lb1index in range(len(lawlist)):
    ...
    lb1.itemconfig(lb1index,background=bgcolorlist[typeoflaw%2])
...
def articlelist(location):
    ...
    for paragraph in file.paragraphs: #生成以条文为元素的列表content
        if paragraph.paragraph_format.alignment == WD_ALIGN_PARAGRAPH.CENTER: continue #居中的都是章或节标题
        if len(paragraph.text)==0: continue
        if "法宝" in paragraph.text:break #北大法宝的文件末尾有广告
        try:
            
            if "条之"  not in paragraph.text[:12]: #不属于之一、之二……
                if numtrans(count) in paragraph.text[:12] and '条'in paragraph.text[:12]: #第一条以及本条首款
                    new_article=True;first_article=True
                    count+=1
                else:
                    new_article=False
            else: #属于之一、之二……，此时应当保持count不变
                if numtrans(count-1) in paragraph.text[:12]: #本条首款
                    new_article=True;first_article=True
                else:
                    new_article=False
...
def but1():
    ...
    for law in lawlist:
        if lb1.select_includes(lawlist.index(law)): #选中了该法律
            selected+=1
            artlist=articlelist(filedir+'//'+law)
            result.write('\n'+law+'\n')
            if multikeyword: #多个关键词
