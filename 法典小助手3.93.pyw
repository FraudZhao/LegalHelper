...
for folder in dirlist_folder2: #不同类型法律放在不同文件夹中 此处的这个临时变量folder尽量不要重构，很麻烦
    ...
    if os.path.isdir(dir_folder2)==False: #法律文件夹下不能直接放文件
        ...
        break
    dirlist_instancesofsingletype=os.listdir(dir_folder2)
    dirlist_instancesofsingletype.sort()#先初排一遍，这样解释1、解释2、解释3就不会乱
    lawlist.append(classification(dirlist_instancesofsingletype, folder))
...
for dirlist_instancesofsingletype in lawlist:
    typeoflaw+=1
    for law in dirlist_instancesofsingletype:
...
def but2():
    ...
    if os.access(dir_result, os.F_OK):
